...
void RemoveReforge(Player* player, uint32 itemguid, bool update)
{
    ...
    Item* invItem = update ? player->GetItemByGuid(ObjectGuid(HIGHGUID_ITEM, itemguid)) : NULL;
    if (invItem)
    ...
    void OnLogin(Player* player, bool firstLogin) override
    {
        ...
        if (result)
        {
            do
            {
                ...
                Item* invItem = player->GetItemByGuid(ObjectGuid(HIGHGUID_ITEM, lowGUID));
                if (invItem)
                    player->_ApplyItemMods(invItem, invItem->GetSlot(), false);
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 melt) override
    {
        ...
        case RESTORE:
            // sender = item guidlow
            {
                if (Item* invItem = player->GetItemByGuid(ObjectGuid(HIGHGUID_ITEM, sender)))
                {
